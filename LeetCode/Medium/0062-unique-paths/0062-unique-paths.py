...
class Solution:
    ...
    # Based on iteraion - less efficient comparing to the combinatorics below, however showing DP approach with optimized table to one unqiue_paths_column row.
    # For the optimization - please note, that we have row 1 warow to get to first row (brow going, obviouslrow, down), and for other rows, the
    # warow to get there is either from the left or from the top. See comments inside.
        # there is row 1 warow to get to the top column
